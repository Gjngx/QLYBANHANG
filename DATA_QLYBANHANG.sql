...
GO
	...
	END
	GO
	CREATE PROC XUATHDTHEONGAY
	@NGAYLAP DATE, @NGAYTT DATE
	AS
	BEGIN
		SELECT  HD.SOHD AS [Số hóa đơn], HD.TONGTIEN AS [Tổng tiền], HD.NGAYLAP AS [Ngày lập], HD.NGAYTHANHTOAN AS [Ngày thanh toán]
		FROM dbo.HOADON AS HD
		WHERE HD.NGAYLAP >= @NGAYLAP AND NGAYTHANHTOAN <= @NGAYTT AND HD.TRANGTHAI = 1 
	END
	GO
	CREATE PROC  XUATTONGTIENTHEONGAY
	@NGAYLAP DATE, @NGAYTT DATE
	AS
	BEGIN
		SELECT TONGTIEN 
	...
	AS
	BEGIN
	DELETE FROM dbo.CTHD WHERE SOHD = @SOHOADON
	...
	SELECT TAIKHOAN , HOTEN , PHANQUYEN FROM dbo.TAIKHOAN WHERE HOTEN LIKE N'%g%'
